        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if ip_raw != "0" and ip_raw != "": 
                    return ip_raw if ip_raw.startswith("http") else f"http://{ip_raw}:8080/video"
        except: pass
        time.sleep(5)
...
def run_sentinel():
    ...
    while True:
        ...
        if motion_detected and is_armed and not is_panic:
            print(f"[ALERT] MOVIMENTO DETECTADO! Area: {total_area}")
            threading.Thread(target=trigger_panic).start()
            is_panic = True 
